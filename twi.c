...
static uint8_t twi_masterBuffer[TWI_BUFFER_LENGTH_RX];
static volatile uint8_t twi_masterBufferIndex;
static volatile uint8_t twi_masterBufferLength;

static uint8_t twi_txBuffer[TWI_BUFFER_LENGTH_TX];
static volatile uint8_t twi_txBufferIndex;
static volatile uint8_t twi_txBufferLength;

static uint8_t twi_rxBuffer[TWI_BUFFER_LENGTH_RX];
...
uint8_t twi_readFrom(uint8_t address, uint8_t* data, uint8_t length, uint8_t sendStop)
{
  ...
  if(TWI_BUFFER_LENGTH_RX < length){
    return 0;
  }
...
uint8_t twi_writeTo(uint8_t address, uint8_t* data, uint8_t length, uint8_t wait, uint8_t sendStop)
{
  ...
  // ensure data will fit into buffer
  if(TWI_BUFFER_LENGTH_TX < length){
    return 1;
  }
...
uint8_t twi_transmit(const uint8_t* data, uint8_t length)
{
  ...
  // ensure data will fit into buffer
  if(TWI_BUFFER_LENGTH_TX < (twi_txBufferLength+length)){
    return 1;
  }
...
ISR(TWI_vect)
{
  switch(TW_STATUS){
    ...
    case TW_SR_GCALL_DATA_ACK: // data received generally, returned ack
      ...
      if(twi_rxBufferIndex < TWI_BUFFER_LENGTH_RX){
